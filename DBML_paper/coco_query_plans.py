...
cnf_queries = pd.read_csv("repo_query/query/coco_voc_query_cnf_gt.csv")
dnf_queries = pd.read_csv("repo_query/query/coco_voc_query_dnf_gt.csv")

A, C, D, _ = data_loader(filename="C:\\Users\\marie\\Documents\\Software\\MILQO\\DBML_paper\\repo_query\\repository\\coco_model_stats_f1_raw_1.csv")
    ...
    for method in MOO_methods:
        for preference_profile in preference_profiles:
            with grb.Env() as env:
                ...
                model = set_MOO_method(model, method=method, objectives=preference_profiles)
                start = time.time()
                model.optimize()
                end = time.time()
                assignment, ordering = model.get_query_plan()
...
DF.to_csv("coco_query_plans.csv")
