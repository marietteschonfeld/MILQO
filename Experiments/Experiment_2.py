...
for query_num, query in enumerate(queries):
    ...
    for method in MOO_methods:
        for preference_type in preference_types:
            item = [query_num, str(query), "model_opt", "CNF", str(preference_type), len(flat_list(query)), method]
            ...
            item = [query_num, str(query), "model_opt", "DNF", str(preference_type), len(flat_list(query)), method]
            model = ModelOpt(A=A, C=C, D=memory, goal='cost', bound=0,
            ...
            item = [query_num, str(query), "order_opt", "CNF", str(preference_type), len(flat_list(query)), method]
            ...
            item = [query_num, str(query), "order_opt", "DNF", str(preference_type), len(flat_list(query)), method]
            model = OrderOpt(A=A, C=C, D=memory, Sel=sel, goal='cost', bound=0,
                             predicates=query, NF="DNF", new_equations=ablation_MAES)
            start = time.time()
            set_MOO_method(model, method, preference_type, p=p, goals=goals, lbs=lbs, ubs=ubs)
